...
export const UpdateCoupon = (props) => {
  const dispatch = useDispatch();
  const bannerID = props.id.toString();
  let timeoutId;
  const listProductInUpdateBanner = [];
  const metaInProductInUpdateBanner = {};
  const listProductOutInForm = [];
  const metaInProductOutUpdateBanner = {};
  ...
  return (
    <div className="w-full p-10 border space-y-5">
      ...
      <div className="flex flex-row-reverse">
        <Button variant="contained" onClick={handleUpdateCoupon}>
          Update
        </Button>
      </div>
    </div>
