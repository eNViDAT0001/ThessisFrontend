...
import { useCategoryIDHandleInAddTree } from "../../../app/hook/CategoryHook";
import { setCategoryIDHandleInAddTree } from "../../../app/slices/CategorySlice";

export const TreeCategoryInAddInAdmin = ({ data, idHandle }) => {
  const [expanded, setExpanded] = useState([]);
  const dispatch = useDispatch();
  const categoryIDHandleInAddTree = useCategoryIDHandleInAddTree();

  const handleTreeClickSpan = async (id) => {
    dispatch(setCategoryIDHandleInAddTree(id));
  };
        ...
        >
          <div
            className={` mr-6 hover:text-pink-500 ${
              treeItem.id === categoryIDHandleInAddTree
                ? "text-pink-500"
                : ""
            }`}
          >
